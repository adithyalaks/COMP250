...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		WordTreeNode current = this.getPrefixNode(word);
		for (int i = current.depth; i < word.length();i++) {
			current = current.createChild(word.charAt(i));
		}
	...
	{
		//   ADD YOUR CODE BELOW HERE
		WordTreeNode prefixNode = this.root;
		for (int i = 0; i < word.length(); i++) {
			if (prefixNode.getChild(word.charAt(i))== null) {
				break;
			}
			else{
	...
	{  
		...
		WordTreeNode prefixNode = this.getPrefixNode(word); 
		
		if (prefix.length() == word.length() && prefixNode.isEndOfWord()){
			return true;
		}
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
		...
		if (prefixNode.depth != prefix.length() ) {
			return list;
		}
		...
		for (int i = 0; i < WordTreeNode.NUMCHILDREN; i++ ) {
			if (prefixNode.children[i] != null) {
				list.addAll(getListPrefixMatches(prefix + (char)i));
			}
		}

		return list;   //  REMOVE THIS STUB
	...
	public class WordTreeNode
	{
		...
		public String toString()
		{
			// ADD YOUR CODE BELOW HERE
			String prefix = "";
			WordTreeNode current = this;
			while (current.depth >= 1) {
				prefix = current.charInParent + prefix;
